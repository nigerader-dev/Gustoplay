...
export const FEATURES = {
  ...
  pageSize: 12,
  /** Расширенная панель «как посчитан подбор» (полезно на старте, можно выключить) */
  scoreDebug: true,
  /** Работать без API: профиль только в браузере */
  localOnlyFallback: true,
  /** Показывать в интерфейсе пометки о партнёрских ссылках (нужно для честности и модерации) */
  affiliateLinks: true,
  /**
   * true — если в папке /covers лежат файлы <slug>.jpg с официальными артами (нужны права!).
   * false — рисуем собственные процедурные обложки (безопасно по авторским правам).
   */
  realCovers: false,
};

export const CONTENT = {
  /** Сколько игр минимум должно быть в пуле, чтобы не считать выдачу «ослабленной» */
  minPool: 8,
  /** Лимит отметок в профиле вкуса (защита от злоупотреблений и размера localStorage) */
  maxMarks: 400,
};
...
export const SECURITY = {
  ...
  /** Максимум запросов в минуту с одного клиента для чувствительных операций (информативно для UI) */
