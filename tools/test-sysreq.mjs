...
check('каждый appid запрошен на обоих языках',
  new Set(requests.filter((r) => r.lang === 'russian').map((r) => r.ids[0])).size === 4
  && new Set(requests.filter((r) => r.lang === 'english').map((r) => r.ids[0])).size === 4);
...
check('собраны все четыре игры', Object.keys(sysreq).length === 4, Object.keys(sysreq).join(', '));
